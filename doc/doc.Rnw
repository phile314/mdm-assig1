...
pima <- read_data('../data/pima.txt', 0.0)
...
\begin{figure}[!ht]
<<perf-comp,fig.keep='high',cache=FALSE>>=
ti_fu_gr <- system.time(replicate(nrep, tree.functional.grow(pima$trxs, pima$trys)))["elapsed"]
ti_im_gr <- system.time(replicate(nrep, tree.grow(pima$trxs, pima$trys)))["elapsed"]

fu_t <- tree.functional.grow(pima$trxs, pima$trys)
im_t <- tree.grow(pima$trxs, pima$trys)

ti_fu_pr <- system.time(replicate(nrep, tree.functional.classify(fu_t, pima$trxs)))["elapsed"]
ti_im_pr <- 0 #TODO broken system.time(replicate(nrep, tree.classify(im_t, pima$trxs)))["elapsed"]
