...
The data set also includes a binary attribute that denotes whether an email is spam or not.
...
As can be seen in figure \ref{plot:perf}, growing times are almost identical, while the functional version is faster at
predicting. Why there is such a difference in the prediction case is an open question.
\begin{figure}[!ht]
...
barplot(c(ti_fu_gr/nrep, ti_im_gr/nrep, ti_fu_pr/nrep, ti_im_pr/nrep), names.arg = c("grow func.", "grow imp.", "predict func.", "predict imp."))
@
\caption{The running time (seconds) of the two implementations, less is better.}
\label{plot:perf}
\end{figure}
